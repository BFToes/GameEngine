...
namespace GameEngine.ECS
{
    public class ArchetypeManager
    {
        ...
        internal Archetype FindOrCreateArchetype(params byte[] indices)
        {
            Array.Sort(indices);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal Archetype InnerFindOrCreateArchetype(byte[] indices)
        {
            Archetype curArchetype = _emptyArchetype;
            for (int i = 0; i < indices.Length; i++) // for each component index
            {
